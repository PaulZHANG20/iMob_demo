...
resized_image = original_image.resize((sw, sh), Image.LANCZOS)
...
content = None
global text_widgets
number_of_columns_a = None
number_of_columns_b = None
line_number2  = None
x111 = None
x211 = None
...
ax1.xaxis.set_major_locator(MaxNLocator(10))  # 例如，最多显示5个刻度
...
def load_():
    global content, number_of_columns_a,number_of_columns_b,line_number2,x111,x211

    # file path for plottin
    ...
    content=pd.read_csv(file,delimiter='\t', skiprows=row_to_skip)
    # print(content)
    #delect NAN
    content = content.dropna(axis=1, how='all')
    #convert to numeric type
    content = content.applymap(lambda x: pd.to_numeric(x, errors='coerce'))
   #for idvd.csv
    ...
    #  Call function
    number_of_columns_a = count_columns_in_file(Path1)
    #print (number_of_columns_a)
    ...
    number_of_columns_b = count_columns_in_file_b(Path1)
    #print(number_of_columns_b)
...
# Plot function
    if content is not None:
        ...
        x111 = content.iloc[0:line_number2-5, 0]
        #print(x111)

        # initialize y_min and y_max
        y_min, y_max = float('inf'), float('-inf')
        #set the first column to display labels
        y1 = content.iloc[0:line_number2 - 5, 1]
        ax1.plot(x111, y1, marker='o', linestyle='', color='blue', markerfacecolor='none', markeredgewidth=1,
                 label=f'Data')

        # use the remaining columns as the y-axis without displaying labels
        for i in range(2, number_of_columns_a):
            y = content.iloc[0:line_number2-5, i]

            # update y_min and y_max
            y_min = min(y_min, y.min())
            y_max = max(y_max, y.max())
            ax1.plot(x111, y, marker='o', linestyle='', color='blue', markerfacecolor='none', markeredgewidth=1)
        ...
        repeated_column = pd.concat([x111] * number_of_columns_a, ignore_index=True)
        ...
        for i in range(1, number_of_columns_a+1):
            y = content.iloc[0:line_number2 - 5, i]
            # format into scientific notation and add to the list
            formatted_values = [f"{value:.2e}" for value in y.values]  # 转换为科学计数法，保留两位小数
        ...
        x211 = content.iloc[line_number2-4:2*line_number2-9, 0]
        #print(x211)

        #set the first column to have labels
        y2 = content.iloc[line_number2 - 4:2 * line_number2 - 9, 1]
        # draw the graph with blue lines and hollow circle markers
        ax2.plot(x211, y2, marker='o', linestyle='', color='blue', markerfacecolor='none', markeredgewidth=1,
                 label=f'Data')
        for i in range(2, number_of_columns_b):
            y3 = content.iloc[line_number2 - 4:2 * line_number2 - 9, i]
            ax2.plot(x211, y3, marker='o', linestyle='', color='blue', markerfacecolor='none', markeredgewidth=1,
                     )
        # set title, axis labels, and legend
        ax2.set_yscale('log')
        ...
        # idvg.csv data
        # set vg data as the first column
        repeated_column = pd.concat([x211] * number_of_columns_b, ignore_index=True)
        ...
        # iterate through columns and add data to the list one by one
        for i in range(1, number_of_columns_b+1):
            y = content.iloc[line_number2 - 4:2 * line_number2 - 9, i]
...
def load_():
    global script_path, file_container, file_content,content_y_nn, number_of_columns_a,number_of_columns_b,line_number2,x111,x211
    ...
    if content_y_nn is not None:
        ...
        # plot first figure
        # set the first column to have labels
        ax1.plot(x111, content_y_nn[ (line_number2 - 5):(2) * (line_number2 - 5)], c='r', label=f'Predicted')
        # set the remaining columns without labels and plot
        for i in range(number_of_columns_a-1):
            ax1.plot(x111, content_y_nn[i*(line_number2-5):(i+1)*(line_number2-5)], c='r')
        ax1.set_title('Output', color='white')
        ax1.set_xlabel('Vds')
        ax1.set_ylabel('Ids')
        ax1.legend()
        ...
        # plot second figure
        # set the first column to have labels
        ax2.plot(x211, content_y_nn[(1 + number_of_columns_a) * (line_number2 - 5):(2+ number_of_columns_a) * (
                    line_number2 - 5)], c='r', label=f'Predicted')
        # set the remaining columns without labels and plot
        for i in range(number_of_columns_b-1):
            ax2.plot(x211, content_y_nn[(i+number_of_columns_a)*(line_number2-5):(i+1+number_of_columns_a)*(line_number2-5)],c='r')
        ax2.set_yscale('log')
        ax2.set_title('Transfer', color='white')
        ax2.set_xlabel('Vgs')
        ax2.set_ylabel('Ids')
